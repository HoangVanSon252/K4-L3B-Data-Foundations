...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for i, part in enumerate(parts):
            text_to_add = part + sep if i < len(parts) - 1 and sep != "" else part
            
            if len(text_to_add) > self.chunk_size:
                if current_chunk:
                    chunks.append(current_chunk)
                    current_chunk = ""
                chunks.extend(self._split(text_to_add, new_separators))
            elif len(current_chunk) + len(text_to_add) > self.chunk_size:
                chunks.append(current_chunk)
